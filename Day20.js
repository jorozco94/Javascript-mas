...
function getFreePodcasts(data){
  // filter for only paid false podcasts
  // return only title, rating, and paid status of podcasts
  const filteredPods = data.filter(el => el.paid === false);
  console.log(filteredPods);
  // need to surround object in parenthesis when returning without using return keyword
  return filteredPods.map(({title, rating, paid}) => ({ title, rating, paid }));
}
